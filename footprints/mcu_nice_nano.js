...
module.exports = {
  ...
  (segment (start ${p.eaxy(
    ...
    const gen_socket_row = (
      ...
      const net_silk_front_left =
        p.reversible && (row_num < 4 || !p.only_required_jumpers)
          ? via_label_right
          : via_label_left;
      const net_silk_front_right =
        p.reversible && (row_num < 4 || !p.only_required_jumpers)
          ? via_label_left
          : via_label_right;
      const net_silk_back_left =
        p.reversible && (row_num < 4 || !p.only_required_jumpers)
          ? via_label_left
          : via_label_right;
      const net_silk_back_right =
        p.reversible && (row_num < 4 || !p.only_required_jumpers)
          ? via_label_right
          : via_label_left;
    ...
    (pad "${socket_hole_num_left}" thru_hole circle (at -7.62 ${
        -12.7 + row_offset_y
      } ${p.r}) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${
        p.reversible && (row_num < 4 || !p.only_required_jumpers)
    ...
    (pad "${via_num_right}" thru_hole circle (at 3.4 ${-12.7 + row_offset_y} ${
        ...
        p.via_drill
      }) (layers "*.Cu" "*.Mask") ${p.reverse_mount ? net_right : net_left})
    (pad "${via_num_left}" thru_hole circle (at -3.4 ${-12.7 + row_offset_y} ${
        p.r
      }) (size ${p.via_size} ${p.via_size}) (drill ${
        p.via_drill
      }) (layers "*.Cu" "*.Mask") ${p.reverse_mount ? net_left : net_right})
      `;
    ...
    (pad "${via_num_left}" smd rect (at -4.58 ${-12.7 + row_offset_y} ${
        ...
        p.reverse_mount ? net_left : net_right
      })
    ...
    (pad "${via_num_right}" smd rect (at 4.58 ${-12.7 + row_offset_y} ${
        ...
        p.reverse_mount ? net_right : net_left
      })
    (pad "${socket_hole_num_left}" smd rect (at 5.48 ${-12.7 + row_offset_y} ${
        p.r
      }) (size 0.6 1.2) (layers "F.Cu" "F.Paste" "F.Mask") ${
    ...
    (pad "${via_num_right}" smd custom (at -4.775 ${-12.7 + row_offset_y} ${
        p.r
      }) (size 0.2 0.2) (layers "B.Cu" "B.Paste" "B.Mask") ${
        p.reverse_mount ? net_right : net_left
      }
      (zone_connect 2)
      (options (clearance outline) (anchor rect))
      ...
      (effects (font (size 1 1) (thickness 0.15)))
        ...
        if (
          ...
          if (
            ...
            socket_row += `
    (fp_text user "${net_silk_back_left}" (at -${
              p.reversible && (row_num < 4 || !p.only_required_jumpers)
      ...
      (effects (font (size 1 1) (thickness 0.15)) (justify mirror))
          ...
          if (
            ...
            socket_row += `
    (fp_text user "${net_silk_back_right}" (at ${
              p.reversible && (row_num < 4 || !p.only_required_jumpers)
  ...
  ${"" /* Controller outline */}
    ...
    const instructions = `
    (fp_text user "R Jumpers, MCU ${
      !p.reverse_mount ? "Up" : "Down"
    }" (at 0 -15.245 ${p.r}) (layer "F.SilkS")
      (effects (font (size 1 1) (thickness 0.15)))
    )
    (fp_text user "L Jumpers, MCU ${
      !p.reverse_mount ? "Up" : "Down"
    ...
    const extra_pins = `
    (pad "25" thru_hole circle (at ${invert_pins ? "" : "-"}5.08 ${
      -12.7 + 25.4
    } ${p.r}) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${p.P101})
    (pad "26" thru_hole circle (at ${invert_pins ? "" : "-"}2.54 ${
      -12.7 + 25.4
    } ${p.r}) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${p.P102})
    (pad "27" thru_hole circle (at 0 ${-12.7 + 25.4} ${
      p.r
    }) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${p.P107})
    `;
    const extra_pins_reversible = `
    (pad "28" thru_hole circle (at ${invert_pins ? "-" : ""}5.08 ${
      -12.7 + 25.4
    } ${p.r}) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${p.P101})
    (pad "29" thru_hole circle (at ${invert_pins ? "-" : ""}2.54 ${
      -12.7 + 25.4
    } ${p.r}) (size 1.7 1.7) (drill 1) (layers "*.Cu" "*.Mask") ${p.P102})
    `;
